...
class RollingStochasticAvailabilityModel:
    ...
    - Rolling-window estimation over the last `window_days`
    - Daily availability window inferred from quantiles (q_start / q_end)
    - Stochastic sampling around rolling means (Gaussian noise)
    - Cached per (resource, date) → one consistent shift per day
    """
    ...
    def __init__(
        ...
        # cache sampled daily windows: (resource, date) -> DailyWindow
        self.daily_cache: Dict[Tuple[str, date], DailyWindow] = {}

        # cache rolling parameters: (resource, date) -> (mu_s, sd_s, mu_e, sd_e)
        self.param_cache: Dict[Tuple[str, date], Optional[Tuple[float, float, float, float]]] = {}
    ...
    def fit_from_csv(
        ...
        # per resource-day derive daily window using quantiles
        daily = (
    ...
    # ---------------------------------------------------------
    # ELIGIBLE RESOURCES (IMPORTANT FIX)
    # ---------------------------------------------------------
    ...
    def eligible_resources(self, t: datetime) -> List[str]:
        ...
        lead to fake "no delay" situations.
        """
        start_date = t.date() - timedelta(days=self.window_days)

        g = self.daily_stats[
            (self.daily_stats["date"] < t.date()) &
            (self.daily_stats["date"] >= start_date)
        ]

        counts = g.groupby("resource")["date"].nunique()
        return counts[counts >= self.min_points].index.tolist()
    ...
    def _sample_window_for_date(
        self,
        resource: str,
        d: date
    ) -> Optional[DailyWindow]:

        cache_key = (resource, d)
        if cache_key in self.daily_cache:
            return self.daily_cache[cache_key]

        params = self._rolling_params(resource, d)
        if params is None:
            return None
    ...
    def next_available(self, resource: str, t: datetime) -> datetime:
        ...
        for day_offset in range(0, 30):
            ...
            cand = datetime.combine(
                d, t.time() if day_offset == 0 else time(0, 0, 0)
            )
        ...
        # fallback (should not happen for eligible resources)
        return t


# UNTEN DIE ALTE VERSION BITTE DA LASSEN WIE ES IST
